...
public class TestMonitoringController {
  ...
  public void test_monitoring_controller_when_patient_is_not_present() throws Exception{
    ...
    final PatientRestController prc = Mockito.mock(PatientRestController.class);
    cc.setPrc(prc);
    ...
    Mockito.when(prc.getPatientById(p1.getId())).thenReturn(new ResponseEntity<>(null,HttpStatus.NOT_FOUND));

    final ResponseEntity<List<String>> result = cc.startPatientMonitoring("A2345");
    if(result.getStatusCode()==HttpStatus.BAD_REQUEST) {
      assertEquals(null, result.getBody());
    }
  ...
  public void test_monitoring_controller_when_sample_is_null() throws Exception {
    ...
    Mockito.when(prc.getPatientById("A2345")).thenReturn(new ResponseEntity<>(p1,HttpStatus.OK));
